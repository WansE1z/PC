...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	printf("Message was received from %s: icmp_seq=%hu ttl=%u time=%.1f\n",
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while(1){
		...
		* funcționalitatea implementată în funcția de ping. */
        ...
        struct icmphdr *ricmp_hdr = (struct icmphdr *)(reply.payload + ICMP_OFF);
		printf("%d. ", i);
		printf("%u.%u.%u.%u", ((ntohl(rip_hdr->saddr) & 0xFF000000) >> 24),
                          ((ntohl(rip_hdr->saddr) & 0x00FF0000)>>16),
                          ((ntohl(rip_hdr->saddr) & 0x0000FF00)>> 8),
                          ((ntohl(rip_hdr->saddr) & 0x000000FF)));
		printf("\n");

        if (ricmp_hdr->type != ICMP_TIME_EXCEEDED) {
            break;
        }
